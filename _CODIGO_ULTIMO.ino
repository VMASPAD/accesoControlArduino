...
String nombreTarjeta = "Bienvenidos";

const byte rowsCount = 3;
const byte columsCount = 3;
char password[] = "5689";  //Contraseña de 4 dígitos
...
const byte rowPins[rowsCount] = { 2, 3, 4 };
const byte columnPins[columsCount] = { 5, 7, 8 };
  ...
  if (mfrc522.PICC_IsNewCardPresent()) {
    ...
    if (mfrc522.PICC_ReadCardSerial()) {
      ...
      if (compareArray(ActualUID, Usuario1) || clave[0] == password[0] && clave[1] == password[1] && clave[2] == password[2] && clave[3] == password[3]) {
        nombreTarjeta = "Bustos";
        Serial.println("Abierto");

        switch (accessControlCount) {
          case 0:
            lcd.clear();
            lcd.print("Bienvenido " + nombreTarjeta);
            delay(200);
            lcd.clear();
            servoMotor.write(90);
            ...
            break;
          default:
            Serial.println("esta en 1 (Abierto)");
            servoMotor.write(0);
